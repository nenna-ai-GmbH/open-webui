...
export function findNewWords(previousText: string, currentText: string): string[] {
	...
	if (newWords.length === 0 && currentText.length > previousText.length) {
		...
		if (addedContent && addedContent.content.trim()) {
			...
			const contextEnd = Math.min(currentText.length, addedContent.offset + addedContent.content.length + 10);
			
			const previousContext = previousText.substring(contextStart, Math.min(previousText.length, contextEnd));
 ...
 */
export function extractIncrementalContent(previousText: string, currentText: string): { content: string; offset: number } | null {
	...
	if (changedContent.length >= minIncrementalSize && 
 ...
 */
export function createContextSnippet(text: string, changeOffset: number, changeLength: number): { content: string; offset: number } {
 ...
 */
export function mergeIncrementalEntities(existingEntities: ExtendedPiiEntity[], newEntities: PiiEntity[]): ExtendedPiiEntity[] {
 ...
 */
export function getSmartDebounceDelay(newWords: string[], incrementalContent: string | null, baseDelay: number = 500): number {
...
export function hasSignificantContent(newWords: string[]): boolean {
	...
	return meaningfulWords.length >= 1 || // At least one meaningful word
		   (newWords.length >= 2); // Or multiple words (even if short)
}
...
export class TypingPauseDetector {
	...
	private handlePauseDetected(
		...
		if (this.hasSignificantTextDifference(currentText)) {
			console.log('PiiDetectionExtension: 🔍 Pause-triggered detection: text differs from last API call');
			onPauseDetected({
				previous: this.lastApiCallText,
				current: currentText
			});
		} else {
			console.log('PiiDetectionExtension: ⏭️ Pause detected but no significant changes, skipping API call');
		}
