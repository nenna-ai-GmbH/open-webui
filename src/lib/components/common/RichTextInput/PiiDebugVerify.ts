...
export function verifyPiiDebug(): void {
  ...
  if (hasWindowDebug) {
    ...
    const methods = ['metrics', 'reset', 'setTracking', 'benchmark', 'config', 'session', 'sync', 'sources', 'help'];
    const availableMethods = methods.filter(method => typeof debugObj[method] === 'function');
    console.log('2. Available methods:', availableMethods.length === methods.length ? '✅ All present' : `⚠️ Missing: ${methods.filter(m => !availableMethods.includes(m)).join(', ')}`);
