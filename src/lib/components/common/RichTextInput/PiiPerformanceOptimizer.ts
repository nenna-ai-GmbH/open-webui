...
export class PiiPerformanceTracker {
  ...
  logMetrics(): void {
    ...
    console.log('📊 State Updates:', metrics.stateUpdates, `(${metrics.stateUpdatesPerSecond.toFixed(2)}/sec)`);
...
export class PiiStateCache {
  ...
  // Entity caching
  cacheEntities(entities: ExtendedPiiEntity[], conversationId?: string, documentHash?: string): void {
  ...
  cachePositionMapping(mapping: PositionMapping, conversationId?: string, documentHash?: string): void {
    const key = this.getCacheKey(conversationId, documentHash);
  ...
  invalidateConversation(conversationId: string): void {
    const conversationKeys = Array.from(this.entityCache.keys())
