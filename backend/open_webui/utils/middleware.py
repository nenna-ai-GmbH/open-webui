...
def unmask_messages_for_rag(
    messages: list[dict], known_entities: list[dict]
) -> list[dict]:
    """
...
async def chat_completion_files_handler(
    ...
    if files := body.get("metadata", {}).get("files", None):
        ...
        log.debug(
            f"Unmasked {len(known_entities)} entities in messages for RAG query generation"
        )
        ...
        log.info(
            f"🔍 RAG Query Generation: Generated {len(queries)} queries with unmasked entities"
        )
