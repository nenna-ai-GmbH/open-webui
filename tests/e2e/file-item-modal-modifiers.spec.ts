...
test('PII text selection and masking workflow', async ({ page }) => {
  ...
  const needsAuth = await page.locator('input[type="email"], input[type="text"][placeholder*="mail" i]').count() > 0;
  ...
  console.log('Page text content after opening file (first 500 chars):', pageTextAfterOpening.substring(0, 500));
  ...
  console.log(`Text search results AFTER opening - aus: ${hasAusAfter}, Siebert: ${hasSiebertAfter}, Berlin: ${hasBerlinAfter}`);
  ...
  if (!pageTextAfterOpening.includes('Siebert') && !pageTextAfterOpening.includes('aus')) {
    ...
    if (await chatInput.isVisible()) {
      ...
      const hasContentNow = pageTextAfterMessage.includes('Siebert') || pageTextAfterMessage.includes('aus') || pageTextAfterMessage.includes('Berlin');
  ...
  try {
    ...
    console.log(`⚠️ Target word "${targetWord}" not found as highlighted, checking for broader highlighting...`);
    ...
    if (highlightCount > 0) {
      console.log(`✅ Found ${highlightCount} highlighted elements (even if not the specific target)`);
      
      // Check if any of our related words got highlighted
      const relatedHighlights = page.locator('.pii-highlight:has-text("Siebert"), .pii-highlight:has-text("Berlin"), .pii-highlight:has-text("aus")');
      ...
      if (!foundAnyPii) {
        console.log('❌ No PII elements found at all - PII masking feature appears not to be working');
  ...
  const allPiiElements = await page.locator('[class*="pii"], .pii-highlight, .pii-masked, .pii-unmasked').count();
