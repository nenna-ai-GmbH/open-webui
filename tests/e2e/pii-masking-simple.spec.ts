...
test.describe('PII Masking Workflow - Simplified', () => {
  test('should select text and apply PII masking', async ({ page }) => {
    ...
    const isLoggedIn = await page.locator('#chat-input').isVisible({ timeout: 5000 }).catch(() => false);
    
    if (!isLoggedIn) {
      console.log('Need to authenticate first. You may need to manually log in or handle auth in beforeEach.');
    ...
    const testText = 'Hello, my name is John Siebert from Berlin. Contact me at john.siebert@example.com';
