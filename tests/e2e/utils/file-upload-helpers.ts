...
export class FileUploadTestHelpers {
      ...
      const completionChecks = [
        ...
        async () => {
          const completionTexts = [
            'uploaded', 'processed', 'done', 'complete', 'fertig', 
            'hochgeladen', 'verarbeitet', 'abgeschlossen'
          ];
  ...
  async verifyFileInList(fileName: string): Promise<void> {
    ...
    if (!found) {
      ...
      console.log(`File ${fileName} not found. Visible text includes: ${visibleText?.substring(0, 500)}`);
   ...
   */
  async waitForFileStatus(fileName: string, expectedStatus: string, timeout = 30000): Promise<void> {
  ...
  async captureUploadConsoleMessages(): Promise<string[]> {
    ...
    this.page.on('console', (msg) => {
      const text = msg.text();
      if (text.includes('PII') || text.includes('upload') || text.includes('file') || text.includes('progress')) {
...
export const FILE_TEST_DATA = {
  ...
  PII_DOCUMENT: '/Users/maxflottmann/Projects/open-webui/tests/e2e/test_files/line.break.entity.docx',
