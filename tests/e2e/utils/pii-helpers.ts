...
export class PiiTestHelpers {
  ...
  async checkPiiHighlighting(): Promise<number> {
    const highlightedElements = this.page.locator('.pii-highlight, [data-pii-label], [data-pii-type]');
  ...
  async startNewChat(): Promise<void> {
    ...
    const newChatButton = this.page.locator('button[aria-label="New Chat"], button:has-text("New Chat")');
  ...
  async loginAs(email: string, password: string): Promise<void> {
    ...
    if (isLoginPage) {
      ...
      if (await emailField.isVisible() && await passwordField.isVisible()) {
        ...
        const signInButton = this.page.locator('button[type="submit"], button:has-text("Sign in"), button:has-text("Login")');
